...
export default function DatasetUpload({ onSuccess }) {
  ...
  const handleSubmit = async () => {
    if (!name || !Object.values(files).every(f => f)) {
    ...
    <div className="bg-slate-800 rounded-xl shadow-2xl p-8">
      ...
      <div className="space-y-6">
        ...
        {['coverage_edges', 'history', 'test_runs', 'changed_files'].map(field => (
          <div key={field}>
            ...
            <input
              ...
              className="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-slate-300 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-purple-600 file:text-white hover:file:bg-purple-700"
            />
          </div>
        ))}
        ...
        <button
          ...
          disabled={uploading || !name || !Object.values(files).every(f => f)}
